    ...
    export class CardHolder extends core.BaseEUI implements ui.IResultDisplay {
      ...
      protected async dealInitState() {
        ...
        for (let i = 0; i < cardAnimNames.length; i++) {
          ...
          cardAnim.armature.getSlot(`card_number_${orientation}`).display = this.getLabelGroup(this._gameData.currentcardindex - currentIndexAnchor + j);
      ...
      protected async flipCards() {
        ...
        for (let i = currentField; i >= 0; i--) {
          ...
          cardAnim.armature.getSlot(`card_number_${orientation}`).display = this.getLabelGroup(this._gameData.currentcardindex - (currentField - i));
