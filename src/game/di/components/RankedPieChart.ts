    ...
    export namespace di {
        export class RankedPieChart extends egret.DisplayObjectContainer {
            ...
            public setRanksAndAnimate(ranks: number[], duration: number = 1000) {
                ...
                var funcCompleted = function (): void {
            ...
            protected renderRanks(ranks: number[], rankSettings: any, emptyRadius: number = 20) {
                ...
                let lastRank = -1;
                let rankTotal = 0;
                for (var i = 0; i < ranksSort.length; i++) {
                    const rank = ranksSort[i];
                    rankTotal += rank;
                    if (rank != lastRank) {
                        rankArray.push(rank);
                        lastRank = rank;
                ...
                if (rankTotal > 0) {
                    ...
                    for (var i = 0; i < ranksCopy.length; i++) {
                        ...
                        let rx = emptyRadius * Math.cos(endAngle);
                        let ry = emptyRadius * Math.sin(endAngle);
                        let rx2 = radius * Math.cos(startAngle);
                        let ry2 = radius * Math.sin(startAngle);
                    ...
                    if (Math.max(bound.width, bound.height) > 0)
                        this.shape.scaleX = this.shape.scaleY = 130 / Math.max(bound.width, bound.height); //limit the max chart size to 130
                }
            }
        }
    }
