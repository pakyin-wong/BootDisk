  ...
  export namespace di {
    export class DiBeadRoad extends ba.BARoadBase {
      ...
      public constructor(width: number, _numRow: number = 1, _numCol: number = 10,  gridSize: number = 30, scale: number = 1, options: any) {
        ...
        let {paddingX, paddingY, gapX, gapY, textSize, diceSize, textPadding, highlightRadius, iconHeight, iconItemYOffset, iconItemColors, showGrid, showOuterGrid } = options;
        this.width = width;
      ...
      protected renderGrid() {
        ...
        if (this.showGrid) {
          ...
          this.grid.graphics.lineStyle(this.gridLine * this.scale, gridColors[0], 1, true);
          if (this.numRow === 1) {
            const iconWidth = (this.width - this.paddingX * 2 - this.gapX * (this.numCol - 1)) / this.numCol;
            let lineX: number = this.paddingX + iconWidth+ this.gapX * 0.5;
      ...
      public initRoadData() {
        ...
        for (let i = 0; i < n; i++) {
          ...
          icon.setByObject({});
          icon.x = this.paddingX + (iconWidth + this.gapX) * (iconIndex % this.numCol);
