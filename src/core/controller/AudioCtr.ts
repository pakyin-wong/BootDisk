  ...
  export namespace core {
    export class AudioCtr {
      ...
      constructor(stage: egret.Stage) {
        logger.l(utils.LoggerTarget.DEBUG, 'AudioCtr is created');
      }
      ...
      public set volumeBGM(vol: number) {
        logger.l(utils.LoggerTarget.DEBUG, `Setting volumeBGM to ${vol}`);
        this._volumeBGM = vol;
        if (this._channelBGM) {
      ...
      public set volumeFX(vol: number) {
        logger.l(utils.LoggerTarget.DEBUG, `Setting volumeFX to ${vol}`);
        this._volumeFX = vol;
        if (this._channelFX) {
      ...
      public set volumeLive(vol: number) {
        logger.l(utils.LoggerTarget.DEBUG, `Setting volumeLive to ${vol}`);
      ...
      public async playSequence(resNameSeq: string[]) {
        ...
        resNameSeq.forEach(name => {
          ...
          chain = chain.then(() => {
            ...
            this._channelFX.volume = this._volumeFX;
            logger.l(utils.LoggerTarget.DEBUG, 'playing', `sn_${name}_${env.voice}_mp3`);
            return new Promise<void>(resolve => {
              this._channelFX.addEventListener(
                egret.Event.SOUND_COMPLETE,
                () => {
                  logger.l(utils.LoggerTarget.DEBUG, 'play end', `sn_${name}_${env.voice}_mp3`);
